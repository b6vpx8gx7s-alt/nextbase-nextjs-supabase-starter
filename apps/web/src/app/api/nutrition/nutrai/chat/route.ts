...
import { withNutriAIContext } from '../context';
import { createAdminClient } from '@/supabase-clients/admin';
import { buildNutriAISystemPrompt } from '@/lib/nutrai-prompt';
import { getNutriAIBusinessContext } from '@/lib/nutrai-business';
import { NUTRAI_TOOLS } from '@/lib/nutrai-tools';

const anthropic = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });

export async function POST(request: NextRequest) {
  return withNutriAIContext(request, async (context, body) => {
    const { message, conversationId } = body as { message: string; conversationId?: string };

    if (!message?.trim()) throw new Error('Message is required');

    const businessContext = await getNutriAIBusinessContext(context.userId);
    const systemPrompt = buildNutriAISystemPrompt(businessContext);

    const tools = NUTRAI_TOOLS.map((tool) => ({
      name: tool.name,
    ...
    if (isNewConversation) {
      const { data: newConv, error: convInsertError } = await supabase
        ...
        .select('id')
        .single();
      if (convInsertError) console.error('[NutriAI] Error creating conversation:', convInsertError.message);
    ...
    try {
      for (let i = 0; i < 5; i++) {
        ...
        for (const block of response.content) {
          if (block.type === 'text') {
            ...
            const tool = NUTRAI_TOOLS.find((t) => t.name === block.name);
            ...
            try {
              ...
              await supabase.from('nutrai_tool_logs').insert({
                business_id: businessContext.businessId,
                user_id: context.userId,
              ...
              isError = true;
              console.error(`[NutriAI] Tool ${block.name} error:`, toolResultContent);
            }
      ...
      if (errMessage.includes('credit balance is too low')) {
        console.error('[NutriAI] Anthropic credit exhausted:', errMessage);
        throw new Error('NutriAI no está disponible temporalmente. Por favor contacta al administrador de la plataforma.');
      }
      throw err;
    }
    ...
    if (conversId) {
      ...
      const { error: convUpdateError } = await supabase
        ...
        .eq('id', conversId);
      if (convUpdateError) console.error('[NutriAI] Error updating conversation:', convUpdateError.message);
    }
