...
export async function createGymClient() {
  ...
  if (!url || !key) {
    const missing = [!url && 'NEXT_PUBLIC_SUPABASE_URL', !key && 'NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY']
      .filter(Boolean)
      .join(', ');
    throw new Error(`[gym/_helpers] Missing env vars: ${missing}`);
  }
...
export function createGymAdminClient() {
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const key = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!url || !key) throw new Error('[gym/_helpers] Missing SUPABASE_SERVICE_ROLE_KEY');
...
export async function getClientAndContext() {
  ...
  if (authError) {
    console.error('[gym/_helpers] auth.getUser error:', authError.message);
  ...
  if (profileRes.error) {
    console.error('[gym/_helpers] profiles query error:', profileRes.error.message, profileRes.error.code);
  }

  if (!profileRes.data?.business_id) {
    console.error('[gym/_helpers] No business_id for user:', user.id);
    return { supabase, ctx: null } as const;
  }
