...
export async function PATCH(request: Request) {
  try {
    ...
    // action === 'accept': aplicar el cambio a routine_data y physio_routine_exercises
    ...
    // Regenerar tabla plana physio_routine_exercises
    await supabase.from('physio_routine_exercises').delete().eq('routine_id', routine.id as string);
    ...
    const { error: insertFlatError } = await supabase
      .from('physio_routine_exercises')
      .insert(flatExercises);
