...
export function ClientCard({ client }: { client: PhysioClient }) {
  const nivelClass = nivelColor[client.nivel_fisico] ?? 'bg-secondary text-secondary-foreground';
  const createdDate = new Date(client.created_at).toLocaleDateString('es-CL');

  return (
    <Link href={`/pacientes/${client.id}`}>
