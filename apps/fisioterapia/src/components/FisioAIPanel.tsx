...
import { type FisioAIBusinessContext } from '@/lib/fisioai-business'
...
export interface FisioAIPanelProps {
  context: FisioAIBusinessContext
}
...
export function FisioAIPanel({ context }: FisioAIPanelProps) {
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      const response = await fetch('/api/fisio/fisioai/chat', {
        method: 'POST',
      ...
      console.error('[FisioAI Error]', message)
      setMessages((prev) => [
        ...prev,
        {
          role: 'assistant',
  ...
  useEffect(() => {
    ...
    window.addEventListener('fisioai:open-with-message', handler)
    return () => window.removeEventListener('fisioai:open-with-message', handler)
  }, [sendMessage])
  ...
  const handleStartRecording = async () => {
    try {
      ...
      recorder.onstop = async () => {
        ...
        try {
          const res = await fetch('/api/fisio/fisioai/transcribe', { method: 'POST', body: fd })
          if (res.ok) {
            const data = await res.json()
            if (data.text) setInput(data.text)
          }
        } catch (err) {
          console.error('[FisioAI] Transcription error:', err)
        }
      ...
      console.error('[FisioAI] Microphone error:', err)
    }
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 14, fontWeight: 500, color: 'white' }}>FisioAI</div>
          <div style={{ fontSize: 12, color: '#9fd9e8', display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            {context.userRole === 'client' ? 'Asistente de paciente' : 'Asistente clínico'}
          </div>
        </div>
        <button
          ...
          <div style={{ textAlign: 'center', padding: '20px 8px 4px' }}>
            ...
            <div style={{ fontSize: 13, color: '#666', marginBottom: 12 }}>
              {context.userRole === 'client'
                ? 'Pregúntame sobre tu plan de ejercicios y progreso'
                : 'Pregúntame sobre tus pacientes, rutinas o alertas clínicas'}
            </div>
      ...
      >
        ...
        <input
          ...
          placeholder={isRecording ? 'Grabando...' : 'Pregúntale a FisioAI...'}
