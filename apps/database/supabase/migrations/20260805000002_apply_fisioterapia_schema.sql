-- ============================================================
-- Apply the fisioterapia schema that was never applied to the DB.
-- The 20260804 migration was committed but never executed against
-- production. Tables with physio_* prefix already exist with a
-- different schema; this migration creates the missing tables and
-- patches the existing ones.
-- ============================================================
...
DO $$ BEGIN
  CREATE POLICY zone_aliases_read ON zone_aliases FOR SELECT TO authenticated USING (true);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
  CREATE POLICY exercise_zones_read ON exercise_zones FOR SELECT TO authenticated USING (true);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
  CREATE POLICY exercise_restrictions_read ON exercise_restrictions FOR SELECT TO authenticated USING (true);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
-- ── Patch physio_routines: add columns the code uses ──────────
ALTER TABLE physio_routines
  ADD COLUMN IF NOT EXISTS semana       integer,
  ADD COLUMN IF NOT EXISTS year         integer,
  ADD COLUMN IF NOT EXISTS routine_data jsonb,
  ADD COLUMN IF NOT EXISTS estado       text DEFAULT 'generada'
                                        CHECK (estado IN ('generada','activa','completada','archivada')),
  ADD COLUMN IF NOT EXISTS generated_at timestamptz DEFAULT now();


-- ── Patch physio_routine_exercises ────────────────────────────
-- Add columns the code uses
ALTER TABLE physio_routine_exercises
  ADD COLUMN IF NOT EXISTS dia              smallint,
  ADD COLUMN IF NOT EXISTS series           smallint,
  ADD COLUMN IF NOT EXISTS reps_o_segundos  integer,
  ADD COLUMN IF NOT EXISTS tempo            text,
  ADD COLUMN IF NOT EXISTS orden            smallint,
  ADD COLUMN IF NOT EXISTS notas_adaptacion text;

-- Fix FK: exercise_id currently points to physio_exercises.id,
-- but the code uses IDs from the new exercises table.
DO $$
DECLARE
  fk_name text;
BEGIN
  ...
  WHERE tc.table_name = 'physio_routine_exercises'
    AND kcu.column_name = 'exercise_id'
    AND kcu2.table_name = 'physio_exercises';

  IF fk_name IS NOT NULL THEN
    EXECUTE 'ALTER TABLE physio_routine_exercises DROP CONSTRAINT ' || quote_ident(fk_name);
    ALTER TABLE physio_routine_exercises
      ADD CONSTRAINT physio_routine_exercises_exercise_id_fkey
...
FROM (VALUES
  ...
  ('Dominada asistida','codo','caution','Tension en flexores del codo bajo codo'),
